...
export class AlertService {
  ...
  public async onResult(result: ProbeResult): Promise<void> {
    ...
    if (!result.success) {
      ...
      if (
        ...
        await this.sendError(result.probeName, state, result, false);
      } else if (
        state.alerting &&
        ...
        state.lastNotificationAt = this.now();
        await this.sendError(result.probeName, state, result, true);
      }
    } else {
